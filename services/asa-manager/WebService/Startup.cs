...
namespace Microsoft.Azure.IoTSolutions.AsaManager.WebService
{
    public class Startup
    {
        ...
        private PersistanceAgent.IAgent persistanceAgent;
        ...
        private void StartAgents()
        {
            ...
            Task.Run(async () =>
            {
               ...
               await this.setupAgent.RunAsync(this.agentsRunState.Token);

            });
            Task.Run(async () =>
            {
                this.telemetryRulesAgent = this.ApplicationContainer.Resolve<TelemetryRulesAgent.IAgent>();
                await this.telemetryRulesAgent.RunAsync(this.agentsRunState.Token);
            });
            Task.Run(async () =>
            {
                this.deviceGroupsAgent = this.ApplicationContainer.Resolve<DeviceGroupsAgent.IAgent>();
                await this.deviceGroupsAgent.RunAsync(this.agentsRunState.Token);
            });
            Task.Run(async () =>
            {
                this.persistanceAgent = this.ApplicationContainer.Resolve<PersistanceAgent.IAgent>();
                await this.persistanceAgent.RunAsync(this.agentsRunState.Token);
            });
        }
