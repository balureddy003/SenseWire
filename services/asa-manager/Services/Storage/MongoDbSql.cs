...
{
    public interface IMongoDbSql
    {
        // Initialize the instance, required before running any other method
        IMongoDbSql Initialize(MongoDbTableConfiguration config);
    ...
    public class MongoDbSql : IMongoDbSql
    {
        // Pattern used to extract URI and Auth key from the connection string
        private const string CONNSTRING_FORMAT = "^mongodb=(?<endpoint>.*);AccountKey=(?<key>.*);$";

        // Wrap SDK methods for testability
        private readonly IMongoDbSqlWrapper mongoSqlWrapper;
        ...
        public MongoDbSql(
            IMongoDbSqlWrapper mongoSqlWrapper,
            ILogger logger)
        {
            this.mongoSqlWrapper = mongoSqlWrapper;
            this.log = logger;
        ...
        public async Task CreateDatabaseAndCollectionsIfNotExistAsync()
        {
           ...
           // this.ParseConnectionString(out var uri, out var authKey);
            ...
            await this.mongoSqlWrapper.CreateDatabaseIfNotExistsAsync(connectionString, this.database);
            await this.mongoSqlWrapper.CreateDocumentCollectionIfNotExistsAsync(connectionString, this.database, this.collection, this.RUs);
        }
        ...
        public async Task<StatusResultServiceModel> PingAsync()
        {
            ...
            {
                //this.ParseConnectionString(out var uri, out var authKey);
                await this.mongoSqlWrapper.ReadDatabaseAsync(connectionString, this.database);
