...
namespace Microsoft.Azure.IoTSolutions.DeviceTelemetry.WebService.v1.Controllers
{
    ...
    public class AlarmsByRuleController : Controller
    {
        ...
        [HttpGet("{id}")]
        [Authorize("ReadAll")]
        public async Task<AlarmListByRuleApiModel> Get(
            [FromRoute] string id,
            [FromQuery] string from,
            [FromQuery] string to,
            [FromQuery] string order,
            ...
            [FromQuery] string devices)
        {
            ...
            return await this.GetAlarmListByRuleHelper(id, from, to, order, skip, limit, deviceIds);
        }
        ...
        private async Task<AlarmListByRuleApiModel> GetAlarmListByRuleHelper(
            string id,
            string from,
            string to,
            string order,
            ...
            string[] deviceIds)
        {
            ...
            List<Alarm> alarmsList =await this.alarmService.ListByRule(
                id,
                fromDate,
                toDate,
                order,
