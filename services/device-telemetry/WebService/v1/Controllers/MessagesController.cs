...
namespace Microsoft.Azure.IoTSolutions.DeviceTelemetry.WebService.v1.Controllers
{
    ...
    public sealed class MessagesController : Controller
    {
        ...
        public async Task<MessageListApiModel> Postmessage([FromBody] Message body)
        {
            string text = JsonConvert.SerializeObject(body);
            var bsonDocument = BsonSerializer.Deserialize<BsonDocument>(text);
            bsonDocument.Set(0, new ObjectId());
            await this.storageClient.postmessages(bsonDocument);
            return null;
        }
