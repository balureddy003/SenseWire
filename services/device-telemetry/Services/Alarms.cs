...
namespace Microsoft.Azure.IoTSolutions.DeviceTelemetry.Services
{
    ...
    public class Alarms : IAlarms
    {
        ...
        {
            BsonDocument doc = this.GetDocumentById(id).Result;

            var alarm = BsonSerializer.Deserialize<Alarm>(doc);
            return new Alarm(doc);
        }

        public async Task<List<Alarm>> List(
            DateTimeOffset? from,
            DateTimeOffset? to,
            string order,
            int skip,
        ...
        public async Task<List<Alarm>> ListByRule(
            string id,
            DateTimeOffset? from,
            DateTimeOffset? to,
            string order,
        ...
        //    BsonDocument document =await this.GetDocumentById(id);
        //    document.Add(STATUS_KEY, status);

        //    Document document1 = await this.storageClient.UpsertDocumentAsync(
        //        this.databaseName,
        //        this.collectionId,
        //        document);

        //    return new Alarm(document1);
        //}

        private async Task<BsonDocument> GetDocumentById(string id)
        {
            InputValidator.Validate(id);
            ...
            List<BsonDocument> documentList =await this.storageClient.QueryDocumentsAsync(
                this.databaseName,
                this.collectionId,
                null,
                query,
            ...
            {
                var alarm = BsonSerializer.Deserialize<BsonDocument>(documentList[0]);
                return alarm;
            }
        ...
        public Task<Alarm> UpdateAsync(string id, string status)
        {
